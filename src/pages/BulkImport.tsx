...
export const BulkImport: React.FC = () => {
  ...
  useEffect(() => {
    if (currentAccount && job && !job.fromEmail) {
      if (currentAccount.fromEmail) {
        updateJobData(currentAccount.id, { fromEmail: currentAccount.fromEmail });
      }
    }
  ...
  const handleInsertImage = () => {
    ...
    let newContent = job.content;
    if (contentRef.current) {
        const start = contentRef.current.selectionStart;
        const end = contentRef.current.selectionEnd;
        newContent = job.content.substring(0, start) + wrapper + job.content.substring(end);
    } else {
        newContent += wrapper;
    }

    updateJobData(currentAccount.id, { content: newContent });
  ...
  const onStart = () => {
    ...
    startJob(currentAccount.id, { apiKey: currentAccount.apiKey || '', secretKey: currentAccount.secretKey });
  };
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto animate-fade-in">
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h1 className="text-3xl font-bold text-foreground tracking-tight">Bulk Send</h1>
        ...
        <Card className="lg:col-span-1 h-fit shadow-md border-t-4 border-t-primary">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea 
                placeholder={`med217623@gmail.com\nmohamedlmalki@yahoo.com\nyassintaziyasin@outlook.com`}
            ...
            <div className="space-y-2">
                ...
                <div className="flex space-x-1">
                    <Dialog open={isImageDialogOpen} onOpenChange={setIsImageDialogOpen}>
                        <DialogTrigger asChild>
                            <Button variant="outline" size="sm" className="h-6 text-[10px] px-2" disabled={isJobActive}>
                    ...
                    <Dialog>
                        ...
                        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
                            ...
                            <div className="border rounded-md p-4 mt-2 min-h-[200px] prose max-w-none dark:prose-invert bg-white text-black">
                                {job.content ? (
              ...
              <Textarea 
                ref={contentRef}
                placeholder="<h1>Hello!</h1><p>This is a bulk message.</p>" 
                    ...
                    <div className="col-span-2 flex items-center justify-center text-xs text-muted-foreground mt-2">
                         {job.status === 'waiting' ? (
                            <span className="flex items-center text-blue-500 font-semibold animate-pulse">
                                <Timer className="w-3 h-3 mr-2" /> Next email in {job.countdown}s...
                            </span>
                        ) : (
